...
class Dataset_Xiexin(Dataset):
    ...
    def __read_data__(self):
        ...
        cols.remove(self.target)
        cols.remove("time")
        cols.remove("roof_type")
        cols.remove("tilt_angle_deg")
        
        # * 将时间戳（time）、特征列（cols）、目标列（target）【一般不用改】
        ...
        # * 变量输入方式（M：多对多 | S：单对单 | MS：多对单）【一般不用改】
        if self.features == 'M' or self.features == 'MS':
            cols_data = df_raw.columns[1:]
            df_data = df_raw[cols_data]
        elif self.features == 'S':
...
class Dataset_Xiexin_SpotTradeLoad(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        df_raw = pd.read_csv(os.path.join(self.root_path,self.data_path), encoding="GB2312")
...
class Dataset_Xiexin_SpotTradePrice(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
...
class Dataset_Xiexin_PowerLoad(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
...
class PSMSegLoader_Original(Dataset):
    ...
    def __len__(self):
        if self.flag == "train":
            ...
            return (self.val.shape[0] - self.win_size) // 1 + 1
        elif (self.flag == 'test'):
            return (self.test.shape[0] - self.win_size) // 1 + 1
        else:
...
class MSLSegLoader_Original(Dataset):
    def __init__(self, args, root_path, win_size, step=100, flag="train"):
        self.flag = flag
        self.step = step
        self.win_size = win_size
...
class SMAPSegLoader_Original(Dataset):
    def __init__(self, args, root_path, win_size, step=100, flag="train"):
        self.flag = flag
        self.step = step
        self.win_size = win_size
...
class SMDSegLoader_Original(Dataset):
    def __init__(self, args, root_path, win_size, step=100, flag="train"):
        self.flag = flag
        self.step = step
        self.win_size = win_size
...
class SWATSegLoader_Original(Dataset):
    def __init__(self, args, root_path, win_size, step=100, flag="train"):
        self.flag = flag
        self.step = step
        self.win_size = win_size
