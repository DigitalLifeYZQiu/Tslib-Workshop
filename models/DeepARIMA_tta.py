...
def arima_log_likelihood(params: np.ndarray, y: np.ndarray, p: int, q: int) -> float:
	"""
...
class ARIMA_Scratch:
	...
	def estimate_arima_parameters(self, y: np.ndarray, p: int, q: int) -> Dict[str, np.ndarray]:
		...
		result = minimize(arima_log_likelihood, initial_params, args=(y, p, q),
	...
	def reconstruct_original_series(self, series: np.ndarray, forecast_diff: np.ndarray) -> np.ndarray:
		"""
		Reconstruct the forecasted original series from differenced for~~ecasts.
...
class Model(nn.Module):
	...
	def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
		...
		self.backbone.fit(np.array(x_enc))
