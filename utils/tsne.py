...
def visualization(X, labels, token_nums,perplexity, path, name):
    ...
    X_sampled, _, y_sampled, _ = train_test_split(X, labels, random_state=42, stratify=labels)

    result = TSNE(n_components=2, random_state=42, perplexity=perplexity).fit_transform(X_sampled)
    ...
    sns.scatterplot(x=result[:, 0], y=result[:, 1], hue=np.array(labels)[y_sampled.astype(int)], hue_order=labels, palette='Set1', s=50, alpha=0.8, linewidth=0
)
    plt.axis('off')
...
def visualization_PCA(X, labels, token_nums, path, name):
    '''
    X: input data from network hidden layer
    labels:
    token_nums: the number of token class (how many kind of tokens)
    '''
    if not os.path.exists(path):
        os.makedirs(path)
