...
mkdir -p logs/TimeBert_SMD_freeze  # 创建日志目录
    ...
    for lr in "${lr_list[@]}"; do
      for lradj in "${lradj_list[@]}"; do
        for epoch in "${epoch_list[@]}"; do
          ...
          # 打印任务信息
          echo "启动任务：seq_len=$seq_len mask_rate=$mask_rate lr=$lr epoch=$epoch → GPU $gpu_id"
...
#            export CUDA_VISIBLE_DEVICES=$gpu_id
            ...
            python -u run.py \
              ...
              --date_record \
              --lradj "$lradj" \
              > "logs/TimeBert_SMD_freeze/TimeBert_SMD_freeze_seq${seq_len}_mask${mask_rate}_lr${lr}-${lradj}_epoch${epoch}.log" 2>&1
          ) &
...
#for seq_len in 40
#do
#for mask_rate in 0.1 0
#do
#for lr in 0.0005 0.00025 0.0001 0.00005 0.000025 0.00001
#do
#for epoch in 3 5 10 15 20
#do
